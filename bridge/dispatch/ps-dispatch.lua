...
if Config.Dispatch ~= 'ps-dispatch' and Config.Dispatch ~= 'auto' then return end
if Config.Dispatch == 'auto' and GetResourceState('ps-dispatch') ~= 'started' then return end
...
function Dispatch.policeAlert(coords, radius, duration, contractKey)
    ...
    exports['ps-dispatch']:CustomAlert({
        ...
        message = 'Suspicious activity nearby.',
        jobs    = Config.PoliceGroups,
        blip    = { sprite = meta.sprite, scale = 1.5, colour = meta.color,
                    flashes = false, text = meta.label, time = (duration or 8) * 1000 },
    })
end
...
function Dispatch.robbery(contractKey, coords, extraData)
    ...
    exports['ps-dispatch']:CustomAlert({
        ...
        jobs    = Config.PoliceGroups,
        blip    = { sprite = meta.sprite, scale = 1.5, colour = meta.color,
                    flashes = false, text = meta.label, time = 15000 },
    })
end
